...
class Wraith::SaveImages
  ...
  def parallel_task(jobs)
    Parallel.each(jobs, :in_threads => wraith.threads, isolation: true) do |_label, _path, width, url, filename, selector, global_before_capture, path_before_capture|
      begin
      ...
      screen_sizes.to_s.split(",").each do |screen_size|
        for attempt in 1..3 do
          begin
            ...
            #driver.manage.window.resize_to(width, height || 5000)
